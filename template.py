...
class Handtracking:
    def __init__(self, mode=False, limit_hands=1, detection=0.7, tracktion=0.7):
        ...
        self.tipIds = [4, 8, 12, 16,
                       20]  # tips of all the fingers in id form as appended in lmslist[id of lm,x-xords,y-cords,0]
        # self.on_or_off = 0
    ...
    def vol(self, frame):
        if len(self.lmslist) != 0:
            prev_x, prev_y = 0, 0
            temp = self.lmslist[8][1:]
            temp2 = self.lmslist[12][1:]
            x1, y1 = temp[0], temp[1]
            ...
            if fingers[0] == 1 and fingers[1] == 1 and fingers[2] == 0 and fingers[3] == 0 and fingers[4] == 0:
                x3 = np.interp(x1, (100, 1280 - 100), (0, 1280))
                y3 = np.interp(y1, (100, 720 - 100), (0, 720))
            ...
            if fingers[1] == 1 and fingers[2] == 1:
                length, frame, lineInfo = self.findDistance(4, 20, frame)
                if length < 20:
                    # cv2.circle(frame, (lineInfo[4], lineInfo[5]), 15, (0, 255, 0), cv2.FILLED)
                    autopy.mouse.click(button=autopy.mouse.Button.RIGHT)
            if fingers[0] == 0 and fingers[2] == 1 and fingers[1] == 0 and fingers[4] == 0:
                tolarence = time.time()
                length, frame, lineInfo = self.findDistance(4, 12, frame)
...
def draw_activate(canvas,frame):
...
def generate_frames():
    ...
    while True:
        if frame_temp is not None:
            if len(frame_temp.shape) == 2:
                frame = cv2.cvtColor(frame_temp, cv2.COLOR_GRAY2BGR)

            resized_frame = cv2.resize(frame_temp, (1080, 720))
...
def main():
    global frame_temp
    global final_frame, draw_frame, gesture
